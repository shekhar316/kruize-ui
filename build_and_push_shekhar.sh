...
echo
echo "################################################################################"
echo "Build and Push Kruize-UI image: ${KRUIZE_UI_DOCKER_IMAGE} to Quay"
echo "Make sure you have logged in to the docker repos"
echo "################################################################################"
echo 

git pull
docker build ${BUILD_PARAMS} --platform linux/amd64 -t ${KRUIZE_UI_DOCKER_IMAGE} -f Dockerfile .
